...
struct ContentView: View {
    ...
    let borderColor = Color.white.opacity(0.20)
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.05) {
                ...
                // closeButton.frame is in its superview's coord system (NSThemeFrame),
                // not in window-base coords — so we must convert explicitly.
                // Both contentLayoutRect.maxY and buttonInBase.maxY are then in the
                // same window-base system (y=0 at bottom), so the subtraction is valid.
                // overlay(alignment:.topTrailing) sits at contentLayoutRect.maxY from bottom,
                // and offset = contentLayoutRect.maxY − buttonTop (negative = move up).
        ...
        HStack(spacing: 0) {
            ...
            Button { zoomIconOut() } label: {
                ...
                Image(systemName: "minus")
                    ...
                    .foregroundStyle(.white.opacity(hoveringIconMinus ? 1.0 : 0.50))
            ...
            Button { zoomIconIn() } label: {
                ...
                Image(systemName: "plus")
                    .font(.system(size: 10, weight: .medium))
                    .foregroundStyle(.white.opacity(hoveringIconPlus ? 1.0 : 0.50))
                    .frame(width: 30, height: 20)
                    .contentShape(Rectangle())
            }
            .buttonStyle(.plain)
    ...
    var linkScaleStepper: some View {
        ...
        HStack(spacing: 0) {
            ...
            Button { zoomLinkOut() } label: {
                ...
                Image(systemName: "minus")
                    .font(.system(size: 10, weight: .medium))
                    .foregroundStyle(.white.opacity(hoveringLinkMinus ? 1.0 : 0.50))
                    .frame(width: 30, height: 20)
                    .contentShape(Rectangle())
            }
            .buttonStyle(.plain)
            ...
            Button { zoomLinkIn() } label: {
                ...
                Image(systemName: "plus")
                    .font(.system(size: 10, weight: .medium))
                    .foregroundStyle(.white.opacity(hoveringLinkPlus ? 1.0 : 0.50))
                    .frame(width: 30, height: 20)
                    .contentShape(Rectangle())
            }
            .buttonStyle(.plain)
